...
// Clean a value read from an .ini: the Win32 profile API does NOT strip inline
// comments, so "mods   ; note" comes back verbatim. Remove a trailing ';' or '#'
// comment (only when it follows whitespace, so folder names containing those
// characters are left intact) and trim surrounding whitespace.
static void ini_clean(char* s){
    for(char* p=s; *p; ++p){
        if((*p==';' || *p=='#') && p!=s && (p[-1]==' ' || p[-1]=='\t')){ *p=0; break; }
    }
    size_t n=s_len(s);
    while(n>0 && (s[n-1]==' ' || s[n-1]=='\t')) s[--n]=0;   // trailing
    size_t lead=0; while(s[lead]==' ' || s[lead]=='\t') ++lead;
    if(lead){ size_t i=0; while(s[lead+i]){ s[i]=s[lead+i]; ++i; } s[i]=0; }
}
...
static void apply_load_order(){
    ...
    while(*p){
        ...
        for(int i=front;i<g_modCount;++i){
            ...
            if(eq && !*x && !*y){
                if(i!=front){ char t[PATHBUF]; s_cpy(t,PATHBUF,g_mods[front]);
...
static void init(HMODULE hSelf){
    ...
    ini_clean(modsDir);
    if(modsDir[0]==0) s_cpy(modsDir, sizeof(modsDir), "mods");  // guard empty/all-comment
